...
func TestBacktrack_CatastrophicTimeout(t *testing.T) {
	...
	var lateAllowance = 500 * time.Millisecond // Large allowance in case machine is slow
...
func TestECMANamedGroup(t *testing.T) {
	...
	_, err := Compile(`(?<title>\w+), yes \k'title'`, 0)
	if err == nil {
		t.Fatal("Expected error")
	}

	re = MustCompile(`(?<title>\w+), yes \k<title>`, 0)
	...
	re = MustCompile(`\k<title>, yes (?<title>\w+)`, 0)
...
func TestFuzzBytes_NoCompile(t *testing.T) {
	...
	for _, c := range testCases {
		...
		t.Run(r, func(t *testing.T) {
			_, err := Compile(r, syntax.Multiline|syntax.Debug)
