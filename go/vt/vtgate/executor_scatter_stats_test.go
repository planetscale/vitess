...
func TestScatterStatsWithSingleScatterQuery(t *testing.T) {
	...
	_, err := executor.Execute(ctx, "TestExecutorResultsExceeded", session, "select * from user", nil)
	require.NoError(t, err)
...
func TestScatterStatsHttpWriting(t *testing.T) {
	...
	_, err = executor.Execute(ctx, "TestExecutorResultsExceeded", session, "select * from user where Id = 15", nil)
	require.NoError(t, err)
	...
	_, err = executor.Execute(ctx, "TestExecutorResultsExceeded", session, query4, nil)
	require.NoError(t, err)
