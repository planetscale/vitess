...
func TestScatterStatsWithSingleScatterQuery(t *testing.T) {
	...
	_, err := executor.Execute(ctx, nil, "TestExecutorResultsExceeded", session, "select * from user", nil, false)
	require.NoError(t, err)
...
func TestScatterStatsHttpWriting(t *testing.T) {
	...
	_, err = executor.Execute(ctx, nil, "TestExecutorResultsExceeded", session, "select * from user where Id = 15", nil, false)
	require.NoError(t, err)
	...
	_, err = executor.Execute(ctx, nil, "TestExecutorResultsExceeded", session, query4, nil, false)
	require.NoError(t, err)
