...
func (e *Executor) getPlan(ctx context.Context, vcursor *vcursorImpl, sql string, comments sqlparser.MarginComments, bindVars map[string]*querypb.BindVariable, qo iQueryOption, logStats *logstats.LogStats) (*engine.Plan, sqlparser.Statement, error) {
	...
	if canMaterialize {
		if materializedPlan, queryID, ok := e.mats.GetPlan(vcursor, statement, bindVars); ok {
			...
			var returnedStmt sqlparser.Statement
			var gen4Plan *engine.Plan
			if boostCmp, isBoostCmp := materializedPlan.Instructions.(*engine.BoostCompare); isBoostCmp {
				gen4Plan, returnedStmt, err = e.cacheAndBuildStatement(ctx, vcursor, query, statement, qo, logStats, stmt, reservedVars, bindVarNeeds)
				if err != nil {
					return nil, returnedStmt, err
				}
				boostCmp.Gen4 = gen4Plan.Instructions
			}
			return materializedPlan, returnedStmt, nil
		}
	}
