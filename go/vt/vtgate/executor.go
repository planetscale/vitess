...
func (e *Executor) getPlan(
	...
	if vcursor.safeSession.boostEnabled() && engine.CanMaterialize(stmt) {
		if materializedPlan, queryID, ok := e.mats.GetPlan(vcursor, stmt, bindVars); ok {
			logStats.BoostQueryID = queryID
			var gen4Plan *engine.Plan
			if boostCmp, isBoostCmp := materializedPlan.Instructions.(*engine.BoostCompare); isBoostCmp {
				gen4Plan, err = e.cacheAndBuildStatement(ctx, vcursor, query, stmt, reservedVars, bindVarNeeds, logStats)
				if err != nil {
					return nil, err
				}
...
func (e *Executor) prepare(ctx context.Context, safeSession *SafeSession, sql string, bindVars map[string]*querypb.BindVariable, logStats *logstats.LogStats) ([]*querypb.Field, error) {
	...
	case sqlparser.StmtDDL, sqlparser.StmtBegin, sqlparser.StmtCommit, sqlparser.StmtRollback, sqlparser.StmtSet, sqlparser.StmtInsert, sqlparser.StmtReplace, sqlparser.StmtUpdate, sqlparser.StmtDelete,
		sqlparser.StmtUse, sqlparser.StmtOther, sqlparser.StmtAnalyze, sqlparser.StmtComment, sqlparser.StmtExplain, sqlparser.StmtFlush:
		return nil, nil
	}
