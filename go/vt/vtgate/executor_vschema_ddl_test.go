...
func TestExecutorAddSequenceDDL(t *testing.T) {
	...
	if _, err = executor.Execute(ctx, nil, "TestExecute", session, stmt, nil, false); err != nil {
		t.Error(err)
	}
	time.Sleep(10 * time.Millisecond)
...
func TestExecutorAddDropVindexDDL(t *testing.T) {
	...
	qr, err := executor.Execute(ctx, nil, "TestExecute", session, "show vschema vindexes on TestExecutor.test", nil, false)
	require.NoError(t, err)
	...
	_, err = executor.Execute(ctx, nil, "TestExecute", session, "show vschema vindexes on TestExecutor.test", nil, false)
	...
	qr, err = executor.Execute(ctx, nil, "TestExecute", session, "show vschema vindexes on TestExecutor.test2", nil, false)
	require.NoError(t, err)
	wantqr = &sqltypes.Result{
		Fields: buildVarCharFields("Columns", "Name", "Type", "Params", "Owner"),
		Rows: [][]sqltypes.Value{
	...
	stmt = "alter vschema on nonexistent drop vindex test_lookup"
	_, err = executor.Execute(ctx, nil, "TestExecute", econtext.NewSafeSession(&vtgatepb.Session{TargetString: "InvalidKeyspace"}), stmt, nil, false)
