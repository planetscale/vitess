...
func TestExecutorSet(t *testing.T) {
	...
	for i, tcase := range testcases {
		t.Run(fmt.Sprintf("%d-%s", i, tcase.in), func(t *testing.T) {
			...
			_, err := executorEnv.Execute(ctx, "TestExecute", session, tcase.in, nil)
			if tcase.err == "" {
...
func TestExecutorSetOp(t *testing.T) {
	...
	for _, tcase := range testcases {
		t.Run(tcase.in, func(t *testing.T) {
			...
			_, err := executor.Execute(ctx, "TestExecute", session, tcase.in, nil)
			require.NoError(t, err)
...
func TestExecutorSetMetadata(t *testing.T) {
	...
	t.Run("Session 2", func(t *testing.T) {
		...
		result, err := executor.Execute(ctx, "TestExecute", session, show, nil)
		...
		gotqr, err := executor.Execute(ctx, "TestExecute", session, show, nil)
		assert.NoError(t, err)
...
func TestSetVar(t *testing.T) {
	...
	_, err := executor.Execute(ctx, "TestSetVar", session, "set @@sql_mode = only_full_group_by", map[string]*querypb.BindVariable{})
	require.NoError(t, err)
	...
	for _, tc := range tcases {
		t.Run(tc.sql, func(t *testing.T) {
			...
			_, err = executor.Execute(ctx, "TestSetVar", session, tc.sql, map[string]*querypb.BindVariable{})
			require.NoError(t, err)
...
func TestSetVarShowVariables(t *testing.T) {
	...
	qr, err := executor.Execute(ctx, "TestSetVar", session, "show variables like 'sql_mode'", map[string]*querypb.BindVariable{})
	require.NoError(t, err)
	...
	for _, tcase := range testcases {
		t.Run(fmt.Sprintf("%s-%s", tcase.sysVar, tcase.val), func(t *testing.T) {
			...
			if tcase.val != "" {
				...
				_, err := e.Execute(ctx, "TestExecutorSetAndSelect", session, setQ, nil)
				require.NoError(t, err)
			}
			...
			qr, err := e.Execute(ctx, "TestExecutorSetAndSelect", session, selectQ, nil)
			require.NoError(t, err)
