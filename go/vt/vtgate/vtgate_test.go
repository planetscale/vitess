...
func TestVTGateBindVarError(t *testing.T) {
	...
	tcases := []struct {
		...
		name: "Execute",
		f: func() error {
			_, _, err := vtg.Execute(ctx, session, "", bindVars)
			return err
		},
	}, {
		name: "ExecuteBatch",
		...
		name: "StreamExecute",
		f: func() error {
			_, err := vtg.StreamExecute(ctx, session, "", bindVars, func(_ *sqltypes.Result) error { return nil })
			return err
		},
	}}
...
func TestErrorIssuesRollback(t *testing.T) {
	...
	session, _, err := vtg.Execute(ctx, &vtgatepb.Session{TargetString: KsTestUnsharded + "@primary"}, "begin", nil)
	if err != nil {
		t.Fatalf("cannot start a transaction: %v", err)
	}
	session, _, err = vtg.Execute(ctx, session, "select id from t1", nil)
	if err != nil {
	...
	sbc.MustFailCodes[vtrpcpb.Code_ABORTED] = 20
	_, _, err = vtg.Execute(ctx, session, "select id from t1", nil)
	if err == nil {
...
func TestMultiInternalSavepointVtGate(t *testing.T) {
	...
	var err error
	session, _, err = vtg.Execute(ctx, session, "begin", nil)
	require.NoError(t, err)
	require.True(t, session.GetAutocommit())
	require.True(t, session.InTransaction)

	// this query goes to multiple shards so internal savepoint will be created.
	session, _, err = vtg.Execute(ctx, session, "insert into sp_tbl(user_id) values (1), (3)", nil)
	require.NoError(t, err)
	require.True(t, session.GetAutocommit())
	require.True(t, session.InTransaction)
	...
	session, _, err = vtg.Execute(ctx, session, "insert into sp_tbl(user_id) values (2), (4)", nil)
	require.NoError(t, err)
	wantQ = []*querypb.BoundQuery{{
		Sql:           "savepoint x",
		BindVariables: map[string]*querypb.BindVariable{},
	...
	_, _, err = vtg.Execute(ctx, session, "insert into sp_tbl(user_id) values (5)", nil)
	require.NoError(t, err)
	wantQ = []*querypb.BoundQuery{{
		Sql: "insert into sp_tbl(user_id) values (:_user_id_0)",
		BindVariables: map[string]*querypb.BindVariable{
