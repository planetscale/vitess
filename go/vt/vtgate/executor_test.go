...
func TestExecutorResultsExceeded(t *testing.T) {
	...
	_, err := executor.Execute(ctx, nil, "TestExecutorResultsExceeded", session, "select * from main1", nil, false)
...
func TestExecutorMaxMemoryRowsExceeded(t *testing.T) {
	...
	for _, test := range testCases {
		...
		_, err = executor.Execute(ctx, nil, "TestExecutorMaxMemoryRowsExceeded", session, test.query, nil, false)
...
func TestExecutorTransactionsNoAutoCommit(t *testing.T) {
	...
	// begin.
	_, err := executor.Execute(ctx, nil, "TestExecute", session, "begin", nil, false)
	require.NoError(t, err)
	...
	// commit.
	_, err = executor.Execute(ctx, nil, "TestExecute", session, "select id from main1", nil, false)
	require.NoError(t, err)
	logStats = testQueryLog(t, executor, logChan, "TestExecute", "SELECT", "select id from main1", 1)
	assert.EqualValues(t, 0, logStats.CommitTime, "logstats: expected zero CommitTime")
	assert.EqualValues(t, "suuid", logStats.SessionUUID, "logstats: expected non-empty SessionUUID")

	_, err = executor.Execute(context.Background(), nil, "TestExecute", session, "commit", nil, false)
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = executor.Execute(ctx, nil, "TestExecute", session, "select id from main1", nil, false)
	require.NoError(t, err)
	_, err = executor.Execute(ctx, nil, "TestExecute", session, "rollback", nil, false)
	require.NoError(t, err)
	wantSession = &vtgatepb.Session{TargetString: "@primary", SessionUUID: "suuid"}
	utils.MustMatch(t, wantSession, session.Session, "session")
	...
	_, err = executor.Execute(ctx, nil, "TestExecute", session, "use @replica", nil, false)
...
func TestDirectTargetRewrites(t *testing.T) {
	...
	_, err := executor.Execute(ctx, nil, "TestExecute", econtext.NewSafeSession(session), sql, map[string]*querypb.BindVariable{}, false)
	require.NoError(t, err)
...
func TestExecutorTransactionsAutoCommit(t *testing.T) {
	...
	_, err = executor.Execute(ctx, nil, "TestExecute", session, "select id from main1", nil, false)
	require.NoError(t, err)
	_, err = executor.Execute(ctx, nil, "TestExecute", session, "commit", nil, false)
	require.NoError(t, err)
...
func TestExecutorDeleteMetadata(t *testing.T) {
	...
	_, err := executor.Execute(ctx, nil, "TestExecute", session, set, nil, false)
	assert.NoError(t, err, "%s error: %v", set, err)

	show := `show vitess_metadata variables like 'app\\_%'`
	result, _ := executor.Execute(ctx, nil, "TestExecute", session, show, nil, false)
	assert.Len(t, result.Rows, 1)

	// Fails if deleting key that doesn't exist
	delQuery := "set @@vitess_metadata.doesn't_exist=''"
	_, err = executor.Execute(ctx, nil, "TestExecute", session, delQuery, nil, false)
	...
	show = `show vitess_metadata variables like 'app\\_%'`
	_, err = executor.Execute(ctx, nil, "TestExecute", session, show, nil, false)
	assert.True(t, topo.IsErrType(err, topo.NoNode))
}
...
func TestExecutorAutocommit(t *testing.T) {
	...
	// autocommit = 1
	_, err = executor.Execute(ctx, nil, "TestExecute", session, "set autocommit=1", nil, false)
	require.NoError(t, err)
	...
	_, err = executor.Execute(ctx, nil, "TestExecute", session, "update main1 set id=1", nil, false)
	require.NoError(t, err)
...
func TestExecutorShowColumns(t *testing.T) {
	...
	for _, query := range queries {
		t.Run(query, func(t *testing.T) {
			_, err := executor.Execute(ctx, nil, "TestExecute", session, query, nil, false)
			require.NoError(t, err)
...
func TestExecutorShow(t *testing.T) {
	...
	for _, query := range []string{"show vitess_keyspaces", "show keyspaces"} {
		qr, err := executor.Execute(ctx, nil, "TestExecute", session, query, nil, false)
		require.NoError(t, err)
	...
	_, err := executor.Execute(ctx, nil, "TestExecute", session, "show variables", nil, false)
	require.NoError(t, err)
	_, err = executor.Execute(ctx, nil, "TestExecute", session, "show collation", nil, false)
	require.NoError(t, err)
	_, err = executor.Execute(ctx, nil, "TestExecute", session, "show collation where `Charset` = 'utf8' and `Collation` = 'utf8_bin'", nil, false)
	require.NoError(t, err)

	_, err = executor.Execute(ctx, nil, "TestExecute", session, "use @primary", nil, false)
	require.NoError(t, err)
	_, err = executor.Execute(ctx, nil, "TestExecute", session, "show tables", nil, false)
	...
	_, err = executor.Execute(ctx, nil, "TestExecute", session, "show create table unknown_table", nil, false)
	assert.EqualErrorf(t, err, wantErrNoTable, "Got: %v. Want: %v", wantErrNoTable)

	// SHOW CREATE table using vschema to find keyspace.
	_, err = executor.Execute(ctx, nil, "TestExecute", session, "show create table user_seq", nil, false)
	require.NoError(t, err)
	lastQuery = sbclookup.Queries[len(sbclookup.Queries)-1].Sql
	...
	_, err = executor.Execute(ctx, nil, "TestExecute", session, fmt.Sprintf("show create table %v.unknown", KsTestUnsharded), nil, false)
	require.NoError(t, err)
	lastQuery = sbclookup.Queries[len(sbclookup.Queries)-1].Sql
	...
	_, err = executor.Execute(ctx, nil, "TestExecute", session, fmt.Sprintf("show keys from %v.unknown", KsTestUnsharded), nil, false)
	require.NoError(t, err)
	lastQuery = sbclookup.Queries[len(sbclookup.Queries)-1].Sql
	wantQuery = "show indexes from `unknown`"
	assert.Equal(t, wantQuery, lastQuery, "Got: %v. Want: %v", lastQuery, wantQuery)

	_, err = executor.Execute(ctx, nil, "TestExecute", session, fmt.Sprintf("show keys from unknown from %v", KsTestUnsharded), nil, false)
	require.NoError(t, err)
	lastQuery = sbclookup.Queries[len(sbclookup.Queries)-1].Sql
	assert.Equal(t, wantQuery, lastQuery, "Got: %v. Want: %v", lastQuery, wantQuery)

	// SHOW INDEX with two different syntax
	_, err = executor.Execute(ctx, nil, "TestExecute", session, fmt.Sprintf("show index from %v.unknown", KsTestUnsharded), nil, false)
	require.NoError(t, err)
	lastQuery = sbclookup.Queries[len(sbclookup.Queries)-1].Sql
	assert.Equal(t, wantQuery, lastQuery, "Got: %v. Want: %v", lastQuery, wantQuery)

	_, err = executor.Execute(ctx, nil, "TestExecute", session, fmt.Sprintf("show index from unknown from %v", KsTestUnsharded), nil, false)
	require.NoError(t, err)
	lastQuery = sbclookup.Queries[len(sbclookup.Queries)-1].Sql
	assert.Equal(t, wantQuery, lastQuery, "Got: %v. Want: %v", lastQuery, wantQuery)

	// SHOW INDEXES with two different syntax
	_, err = executor.Execute(ctx, nil, "TestExecute", session, fmt.Sprintf("show indexes from %v.unknown", KsTestUnsharded), nil, false)
	require.NoError(t, err)
	lastQuery = sbclookup.Queries[len(sbclookup.Queries)-1].Sql
	assert.Equal(t, wantQuery, lastQuery, "Got: %v. Want: %v", lastQuery, wantQuery)

	_, err = executor.Execute(ctx, nil, "TestExecute", session, fmt.Sprintf("show indexes from unknown from %v", KsTestUnsharded), nil, false)
	require.NoError(t, err)
	lastQuery = sbclookup.Queries[len(sbclookup.Queries)-1].Sql
	assert.Equal(t, wantQuery, lastQuery, "Got: %v. Want: %v", lastQuery, wantQuery)

	// SHOW EXTENDED {INDEX | INDEXES | KEYS}
	_, err = executor.Execute(ctx, nil, "TestExecute", session, fmt.Sprintf("show extended index from unknown from %v", KsTestUnsharded), nil, false)
	require.NoError(t, err)
	lastQuery = sbclookup.Queries[len(sbclookup.Queries)-1].Sql
	assert.Equal(t, wantQuery, lastQuery, "Got: %v. Want: %v", lastQuery, wantQuery)

	_, err = executor.Execute(ctx, nil, "TestExecute", session, fmt.Sprintf("show extended indexes from unknown from %v", KsTestUnsharded), nil, false)
	require.NoError(t, err)
	lastQuery = sbclookup.Queries[len(sbclookup.Queries)-1].Sql
	assert.Equal(t, wantQuery, lastQuery, "Got: %v. Want: %v", lastQuery, wantQuery)

	_, err = executor.Execute(ctx, nil, "TestExecute", session, fmt.Sprintf("show extended keys from unknown from %v", KsTestUnsharded), nil, false)
	require.NoError(t, err)
	lastQuery = sbclookup.Queries[len(sbclookup.Queries)-1].Sql
	assert.Equal(t, wantQuery, lastQuery, "Got: %v. Want: %v", lastQuery, wantQuery)
	...
	_, err = executor.Execute(ctx, nil, "TestExecute", session, "show create table unknown", nil, false)
	require.NoError(t, err)

	_, err = executor.Execute(ctx, nil, "TestExecute", session, "show full columns from table1", nil, false)
	require.NoError(t, err)

	// Reset target string so other tests dont fail.
	session.TargetString = "@primary"
	_, err = executor.Execute(ctx, nil, "TestExecute", session, fmt.Sprintf("show full columns from unknown from %v", KsTestUnsharded), nil, false)
	require.NoError(t, err)
	...
	for _, sql := range []string{"show session status", "show session status like 'Ssl_cipher'"} {
		qr, err = executor.Execute(ctx, nil, "TestExecute", session, sql, nil, false)
		require.NoError(t, err)
		wantqr = &sqltypes.Result{
			Fields: []*querypb.Field{
	...
	_, err = executor.Execute(ctx, nil, "TestExecute", session, fmt.Sprintf("show full columns from %v.table1", KsTestUnsharded), nil, false)
	require.NoError(t, err)
...
func TestExecutorShowTargeted(t *testing.T) {
	...
	for _, sql := range queries {
		_, err := executor.Execute(ctx, nil, "TestExecutorShowTargeted", session, sql, nil, false)
		require.NoError(t, err)
...
func TestExecutorShowFromSystemSchema(t *testing.T) {
	...
	_, err := executor.Execute(ctx, nil, "TestExecutorShowFromSystemSchema", session, "show tables", nil, false)
	require.NoError(t, err)
}
...
func TestExecutorUse(t *testing.T) {
	...
	for i, stmt := range stmts {
		_, err := executor.Execute(ctx, nil, "TestExecute", session, stmt, nil, false)
		if err != nil {
			t.Error(err)
		}
	...
	_, err := executor.Execute(ctx, nil, "TestExecute", econtext.NewSafeSession(&vtgatepb.Session{}), "use 1", nil, false)
	wantErr := "syntax error at position 6 near '1'"
	if err == nil || err.Error() != wantErr {
		t.Errorf("got: %v, want %v", err, wantErr)
	}

	_, err = executor.Execute(ctx, nil, "TestExecute", econtext.NewSafeSession(&vtgatepb.Session{}), "use UnexistentKeyspace", nil, false)
...
func TestExecutorComment(t *testing.T) {
	...
	for _, stmt := range stmts {
		gotResult, err := executor.Execute(ctx, nil, "TestExecute", econtext.NewSafeSession(&vtgatepb.Session{TargetString: KsTestUnsharded}), stmt, nil, false)
		if err != nil {
			t.Error(err)
		}
...
func TestExecutorDDL(t *testing.T) {
	...
	for _, stmt := range stmts {
		for _, tc := range tcs {
			...
			stmtType := "DDL"
			_, err := executor.Execute(ctx, nil, "TestExecute", econtext.NewSafeSession(&vtgatepb.Session{TargetString: tc.targetStr}), stmt, nil, false)
	...
	for _, stmt := range stmts2 {
		...
		sbc2.ExecCount.Store(0)
		sbclookup.ExecCount.Store(0)
		_, err := executor.Execute(ctx, nil, "TestExecute", econtext.NewSafeSession(&vtgatepb.Session{TargetString: ""}), stmt.input, nil, false)
		if stmt.hasErr {
	...
	for _, stmt := range stmts {
		for _, mode := range []string{"allow", "disallow"} {
			t.Run(stmt+mode, func(t *testing.T) {
				...
				_, err := executor.Execute(ctx, nil, mName, econtext.NewSafeSession(&vtgatepb.Session{TargetString: KsTestUnsharded}), stmt, nil, false)
				if mode == "allow" {
					require.NoError(t, err)
...
func TestExecutorVindexDDLACL(t *testing.T) {
	...
	_, err := executor.Execute(ctxRedUser, nil, "TestExecute", session, stmt, nil, false)
	require.EqualError(t, err, `User 'redUser' is not authorized to perform vschema operations`)

	_, err = executor.Execute(ctxBlueUser, nil, "TestExecute", session, stmt, nil, false)
...
func TestExecutorUnrecognized(t *testing.T) {
	...
	_, err := executor.Execute(ctx, nil, "TestExecute", econtext.NewSafeSession(&vtgatepb.Session{}), "invalid statement", nil, false)
...
func TestExecutorDeniedErrorNoBuffer(t *testing.T) {
	...
	startExec := time.Now()
	_, err := executor.Execute(ctx, nil, "TestExecutorDeniedErrorNoBuffer", session, "select * from user", nil, false)
...
func getPlanCached(t *testing.T, ctx context.Context, e *Executor, vcursor *econtext.VCursorImpl, sql string, comments sqlparser.MarginComments, bindVars map[string]*querypb.BindVariable, skipQueryPlanCache bool) (*engine.Plan, *logstats.LogStats) {
	...
	require.NoError(t, err)
	plan, err := e.getPlan(context.Background(), vcursor, sql, stmt, comments, bindVars, reservedVars /* normalize */, e.normalize, logStats, false)
	require.NoError(t, err)
...
func TestGetPlanPriority(t *testing.T) {
	...
	for _, aTestCase := range testCases {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			_, err = r.getPlan(context.Background(), vCursor, testCase.sql, stmt, makeComments("/* some comment */"), map[string]*querypb.BindVariable{}, nil, true, logStats, false)
...
func TestExecutorMaxPayloadSizeExceeded(t *testing.T) {
	...
	for _, query := range testMaxPayloadSizeExceeded {
		_, err := executor.Execute(context.Background(), nil, "TestExecutorMaxPayloadSizeExceeded", session, query, nil, false)
	...
	for _, query := range testMaxPayloadSizeOverride {
		_, err := executor.Execute(context.Background(), nil, "TestExecutorMaxPayloadSizeWithOverride", session, query, nil, false)
...
func TestExecutorClearsWarnings(t *testing.T) {
	...
	_, err := executor.Execute(context.Background(), nil, "TestExecute", session, "select 42", nil, false)
	require.NoError(t, err)
...
func TestServingKeyspaces(t *testing.T) {
	...
	result, err := executor.Execute(ctx, nil, "TestServingKeyspaces", econtext.NewSafeSession(&vtgatepb.Session{}), "select keyspace_name from dual", nil, false)
	require.NoError(t, err)
...
func TestExecutorAnalyze(t *testing.T) {
	...
	for _, tc := range tcs {
		t.Run(tc.targetStr, func(t *testing.T) {
			...
			_, err := executor.Execute(context.Background(), nil, "TestExecute", econtext.NewSafeSession(&vtgatepb.Session{TargetString: tc.targetStr}), stmt, nil, false)
			require.NoError(t, err)
...
func TestExecutorCallProc(t *testing.T) {
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := executor.Execute(context.Background(), nil, "TestExecute", econtext.NewSafeSession(&vtgatepb.Session{TargetString: tc.targetStr}), "CALL proc()", nil, false)
			if tc.hasNoKeyspaceErr {
...
func TestExecutorTempTable(t *testing.T) {
	...
	session := econtext.NewSafeSession(&vtgatepb.Session{TargetString: KsTestUnsharded})
	_, err := executor.Execute(ctx, nil, "TestExecutorTempTable", session, creatQuery, nil, false)
	require.NoError(t, err)
	...
	_, err = executor.Execute(ctx, nil, "TestExecutorTempTable", session, "select * from temp_t", nil, false)
	require.NoError(t, err)
...
func TestExecutorShowVitessMigrations(t *testing.T) {
	...
	session := econtext.NewSafeSession(&vtgatepb.Session{TargetString: "TestExecutor"})
	_, err := executor.Execute(ctx, nil, "", session, showQuery, nil, false)
	require.NoError(t, err)
...
func TestExecutorVExplainQueries(t *testing.T) {
	...
	qr, err := executor.Execute(ctx, nil, "TestExecutorVExplainQueries", session, "vexplain queries select * from user where name = 'apa'", nil, false)
	require.NoError(t, err)
	txt := fmt.Sprintf("%v\n", qr.Rows)
...
func TestExecutorStartTxnStmt(t *testing.T) {
	...
	for _, tcase := range tcases {
		t.Run(tcase.beginSQL, func(t *testing.T) {
			_, err := executor.Execute(ctx, nil, "TestExecutorStartTxnStmt", session, tcase.beginSQL, nil, false)
			require.NoError(t, err)

			assert.Equal(t, tcase.expTxAccessMode, session.GetOrCreateOptions().TransactionAccessMode)

			_, err = executor.Execute(ctx, nil, "TestExecutorStartTxnStmt", session, "rollback", nil, false)
			require.NoError(t, err)
...
func TestExecutorPrepareExecute(t *testing.T) {
	...
	// prepare statement.
	_, err := executor.Execute(context.Background(), nil, "TestExecutorPrepareExecute", session, "prepare prep_user from 'select * from user where id = ?'", nil, false)
	require.NoError(t, err)
	...
	_, err = executor.Execute(context.Background(), nil, "TestExecutorPrepareExecute", session, "set @udv_query = 'select * from user where id in (?,?,?)'", nil, false)
	require.NoError(t, err)

	_, err = executor.Execute(context.Background(), nil, "TestExecutorPrepareExecute", session, "prepare prep_user2 from @udv_query", nil, false)
	require.NoError(t, err)
	...
	_, err = executor.Execute(context.Background(), nil, "TestExecutorPrepareExecute", session, "prepare prep_user2 from 'select'", nil, false)
	require.Error(t, err)
	...
	_, err = executor.Execute(context.Background(), nil, "TestExecutorPrepareExecute", session, "prepare prep_user from @foo", nil, false)
	require.Error(t, err)
	require.Nil(t, session.PrepareStatement["prep_user"]) // prepared statement is cleared from the session.

	// empty prepared query
	_, err = executor.Execute(context.Background(), nil, "TestExecutorPrepareExecute", session, "prepare prep_user from ''", nil, false)
	require.Error(t, err)
}
...
func TestExecutorSettingsInTwoPC(t *testing.T) {
	...
	for _, tcase := range tcases {
		t.Run(fmt.Sprintf("%v", tcase.sqls), func(t *testing.T) {
			...
			// start transaction
			_, err := executor.Execute(ctx, nil, "TestExecutorSettingsInTwoPC", session, "begin", nil, false)
			require.NoError(t, err)

			// execute queries
			for _, sql := range tcase.sqls {
				_, err = executor.Execute(ctx, nil, "TestExecutorSettingsInTwoPC", session, sql, nil, false)
				require.NoError(t, err)
			}

			// commit 2pc
			_, err = executor.Execute(ctx, nil, "TestExecutorSettingsInTwoPC", session, "commit", nil, false)
			require.NoError(t, err)
...
func TestExecutorTruncateErrors(t *testing.T) {
	...
	_, err := executor.Execute(ctx, nil, "TestExecute", session, "invalid statement", nil, false)
...
func TestExecutorFlushStmt(t *testing.T) {
	...
	for _, tc := range tcs {
		t.Run(tc.query+tc.targetStr, func(t *testing.T) {
			_, err := executor.Execute(context.Background(), nil, "TestExecutorFlushStmt", econtext.NewSafeSession(&vtgatepb.Session{TargetString: tc.targetStr}), tc.query, nil, false)
			if tc.expectedErr == "" {
				require.NoError(t, err)
			} else {
				require.Error(t, err)
...
func exec(executor *Executor, session *econtext.SafeSession, sql string) (*sqltypes.Result, error) {
	return executor.Execute(context.Background(), nil, "TestExecute", session, sql, nil, false)
}
