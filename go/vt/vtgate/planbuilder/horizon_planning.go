...
func (hp *horizonPlanning) planHorizon(ctx *plancontext.PlanningContext, plan logicalPlan, truncateColumns bool) (logicalPlan, error) {
	...
	var err error
	hp.qp, err = abstract.CreateQPFromSelect(ctx, hp.sel)
	if err != nil {
		return nil, err
	}
...
func checkIfAlreadyExists(expr *sqlparser.AliasedExpr, node sqlparser.SelectStatement, semTable *semantics.SemTable) int {
	...
	for i, selectExpr := range sel.SelectExprs {
		...
		if semTable.EqualsExpr(selectExpr.Expr, expr.Expr) {
...
func checkOrderExprCanBePlannedInScatter(ctx *plancontext.PlanningContext, plan *routeGen4, order abstract.OrderBy, hasStar bool) error {
	if !hasStar {
		return nil
	}
...
func findExprInOrderedAggr(ctx *plancontext.PlanningContext, plan *orderedAggregate, order abstract.OrderBy) (keyCol int, weightStringCol int, found bool) {
