...
func TestTxConnBegin(t *testing.T) {
	...
	_, errors := sc.ExecuteMultiShard(ctx, nil, rss0, queries, safeSession, false, false, nil)
...
func TestTxConnCommitFailure(t *testing.T) {
	...
	sc.ExecuteMultiShard(ctx, nil, rssm[0], queries, session, false, false, nil)
	...
	sc.ExecuteMultiShard(ctx, nil, rssm[1], queries, session, false, false, nil)
	wantSession = vtgatepb.Session{
		InTransaction: true,
		ShardSessions: []*vtgatepb.Session_ShardSession{{
			Target: &querypb.Target{
	...
	sc.ExecuteMultiShard(ctx, nil, rssa, threeQueries, session, false, false, nil)
	wantSession = vtgatepb.Session{
		InTransaction: true,
		ShardSessions: []*vtgatepb.Session_ShardSession{{
			Target: &querypb.Target{
...
func TestTxConnCommitFailureAfterNonAtomicCommitMaxShards(t *testing.T) {
	...
	for i := 0; i < 18; i++ {
		sc.ExecuteMultiShard(ctx, nil, rssm[i], queries, session, false, false, nil)
...
func TestTxConnCommitSuccess(t *testing.T) {
	...
	// Sequence the executes to ensure commit order
	session := NewSafeSession(&vtgatepb.Session{InTransaction: true})
	sc.ExecuteMultiShard(ctx, nil, rss0, queries, session, false, false, nil)
	wantSession := vtgatepb.Session{
		InTransaction: true,
		ShardSessions: []*vtgatepb.Session_ShardSession{{
			Target: &querypb.Target{
	...
	utils.MustMatch(t, &wantSession, session.Session, "Session")
	sc.ExecuteMultiShard(ctx, nil, rss01, twoQueries, session, false, false, nil)
	wantSession = vtgatepb.Session{
		InTransaction: true,
		ShardSessions: []*vtgatepb.Session_ShardSession{{
			Target: &querypb.Target{
...
func TestTxConnReservedOn2ShardTxOn1ShardAndCommit(t *testing.T) {
	...
	_, errs := sc.ExecuteMultiShard(ctx, nil, rss1, queries, session, false, false, nil)
	require.Empty(t, errs)
	_, errs = sc.ExecuteMultiShard(ctx, nil, rss0, queries, session, false, false, nil)
	require.Empty(t, errs)
...
func TestTxConnCommitOrderFailure1(t *testing.T) {
	...
	sc.ExecuteMultiShard(ctx, nil, rss1, queries, session, false, false, nil)
...
func TestTxConnCommitOrderFailure2(t *testing.T) {
	...
	// Sequence the executes to ensure commit order
	session := NewSafeSession(&vtgatepb.Session{InTransaction: true})
	sc.ExecuteMultiShard(context.Background(), nil, rss1, queries, session, false, false, nil)

	session.SetCommitOrder(vtgatepb.CommitOrder_PRE)
	sc.ExecuteMultiShard(context.Background(), nil, rss0, queries, session, false, false, nil)
