...
func TestRecheckPrimaryHealth(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			// acker's analysis (via BeforeAnalyses dependency), so
			// checkIfAlreadyFixed finds it and returns alreadyFixed=false → proceed.
...
func TestRecoverShardAnalyses(t *testing.T) {
	...
	// so they require ordered execution. ReplicationStopped has BeforeAnalyses:
	// [PrimarySemiSyncBlocked], so it also requires ordered execution and runs before the
	// shard-wide actions. ReplicaIsWritable has no dependencies and runs concurrently.
	...
	// Ordered recoveries must come first, in their original input order.
	// ReplicationStopped requires ordered execution (BeforeAnalyses: PrimarySemiSyncBlocked).
	...
	require.ElementsMatch(t, []inst.AnalysisCode{inst.ReplicaIsWritable}, order[3:])
}
