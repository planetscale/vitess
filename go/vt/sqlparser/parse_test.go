...
var (
	validSQL = []struct {
		...
		input: "SELECT a.id, a.agent_id AS parent_id, a.rebate_rate_plan, a.reward_rate_plan, total_gs.category, total_gs.partner, total_gs.total_win_or_lose, total_gs.total_valid_amount, total_gs.total_game_fee, total_gs.agent_rebate_rate AS agent_rebate_rate, total_gs.agent_reward_rate AS agent_reward_rate, total_gs.agent_rebate_fee_percentage, total_gs.agent_reward_fee_percentage, total_gs.parent_rebate_rate AS parent_rebate_rate, total_gs.parent_reward_rate AS parent_reward_rate, total_gs.parent_rebate_fee_percentage, total_gs.parent_reward_fee_percentage, total_bs.bonus_cost AS bonus_cost, total_bs.agent_bonus_cost_rate AS bonus_cost_rate, total_bs.take_bonus_cost AS take_bonus_cost, total_bs.event_cost AS event_cost, total_bs.agent_event_cost_rate AS event_cost_rate, total_bs.take_event_cost AS take_event_cost, total_bs.cost AS cost, total_bs.take_cost AS take_cost, total_bs.parent_take_bonus_cost AS parent_take_bonus_cost, total_bs.parent_bonus_cost_rate AS parent_bonus_cost_rate, total_bs.parent_take_event_cost AS parent_take_event_cost, total_bs.parent_event_cost_rate AS parent_event_cost_rate, total_bs.parent_take_cost AS parent_take_cost, CASE WHEN total_gs.rebate_rate_plan = 'category' AND total_gs.category = 0 AND total_gs.partner = 0 THEN SUM( CASE WHEN total_gs.category != 0 AND total_gs.partner = 0 THEN take_rebate ELSE 0 END ) OVER (PARTITION BY total_gs.agent_id) WHEN total_gs.rebate_rate_plan = 'category_partner' AND total_gs.category = 0 AND total_gs.partner = 0 THEN SUM( CASE WHEN total_gs.category != 0 AND total_gs.partner != 0 THEN take_rebate ELSE 0 END ) OVER (PARTITION BY total_gs.agent_id) ELSE take_rebate END AS actual_take_rebate, CASE WHEN total_gs.reward_rate_plan = 'category' AND total_gs.category = 0 AND total_gs.partner = 0 THEN SUM( CASE WHEN total_gs.category != 0 AND total_gs.partner = 0 THEN total_gs.take_reward ELSE 0 END ) OVER (PARTITION BY total_gs.agent_id) WHEN total_gs.reward_rate_plan = 'category_partner' AND total_gs.category = 0 AND total_gs.partner = 0 THEN SUM( CASE WHEN total_gs.category != 0 AND total_gs.partner != 0 THEN take_reward ELSE 0 END ) OVER (PARTITION BY total_gs.agent_id) ELSE take_reward END AS actual_take_reward, CASE WHEN total_gs.rebate_rate_plan = 'category' AND total_gs.category = 0 AND total_gs.partner = 0 THEN SUM( CASE WHEN total_gs.category != 0 AND total_gs.partner = 0 THEN take_rebate_game_fee ELSE 0 END ) OVER (PARTITION BY total_gs.agent_id) WHEN total_gs.rebate_rate_plan = 'category_partner' AND total_gs.category = 0 AND total_gs.partner = 0 THEN SUM( CASE WHEN total_gs.category != 0 AND total_gs.partner != 0 THEN take_rebate_game_fee ELSE 0 END ) OVER (PARTITION BY total_gs.agent_id) ELSE take_rebate_game_fee END AS actual_take_rebate_game_fee, CASE WHEN total_gs.reward_rate_plan = 'category' AND total_gs.category = 0 AND total_gs.partner = 0 THEN SUM( CASE WHEN total_gs.category != 0 AND total_gs.partner = 0 THEN take_reward_game_fee ELSE 0 END ) OVER (PARTITION BY total_gs.agent_id) WHEN total_gs.reward_rate_plan = 'category_partner' AND total_gs.category = 0 AND total_gs.partner = 0 THEN SUM( CASE WHEN total_gs.category != 0 AND total_gs.partner != 0 THEN take_reward_game_fee ELSE 0 END ) OVER (PARTITION BY total_gs.agent_id) ELSE take_reward_game_fee END AS actual_take_reward_game_fee FROM agents AS a LEFT JOIN ( SELECT cal_gs.agent_id, cal_gs.rebate_rate_plan, cal_gs.reward_rate_plan, cal_gs.category, cal_gs.partner, cal_gs.total_win_or_lose, cal_gs.total_valid_amount, cal_gs.total_game_fee, a_rebate_r.percentage AS agent_rebate_rate, a_reward_r.percentage AS agent_reward_rate, a_rebate_r.fee_percentage AS agent_rebate_fee_percentage, a_reward_r.fee_percentage AS agent_reward_fee_percentage, COALESCE(p_rebate_r.percentage, 100) AS parent_rebate_rate, COALESCE(p_reward_r.percentage, 100) AS parent_reward_rate, p_rebate_r.fee_percentage AS parent_rebate_fee_percentage, p_reward_r.fee_percentage AS parent_reward_fee_percentage, ROUND( - total_win_or_lose * a_reward_r.percentage / 100, 2 ) AS take_reward, ROUND( total_valid_amount * a_rebate_r.percentage / 100, 2 ) AS take_rebate, CASE WHEN - cal_gs.total_win_or_lose <= 0 THEN 0 ELSE ROUND( - cal_gs.total_win_or_lose * a_rebate_r.fee_percentage / 100, 2 ) END AS take_rebate_game_fee, CASE WHEN - cal_gs.total_win_or_lose <= 0 THEN 0 ELSE ROUND( - cal_gs.total_win_or_lose * a_reward_r.fee_percentage / 100, 2 ) END AS take_reward_game_fee FROM ( SELECT a.id AS agent_id, a.agent_id AS parent_id, a.rebate_rate_plan, a.reward_rate_plan, COALESCE(gs.category, 0) AS category, COALESCE(gs.partner, 0) AS partner, COALESCE(SUM(gs.win_or_lose), 0) AS total_win_or_lose, COALESCE(SUM(gs.valid_amount), 0) AS total_valid_amount, CASE WHEN - SUM(gs.win_or_lose) <= 0 THEN 0 ELSE - SUM(gs.win_or_lose) END AS total_game_fee FROM game_stats AS gs JOIN users AS u ON gs.user_id = u.id JOIN agents AS a ON u.real_agent_id = a.id WHERE gs.day = '2024-01-01' GROUP BY a.id, gs.category, gs.partner WITH ROLLUP ) AS cal_gs LEFT JOIN agent_rates AS a_rebate_r ON cal_gs.agent_id = a_rebate_r.agent_id AND a_rebate_r.benefit_type = 1 AND a_rebate_r.category = cal_gs.category AND a_rebate_r.partner = cal_gs.partner LEFT JOIN agent_rates AS a_reward_r ON cal_gs.agent_id = a_reward_r.agent_id AND a_reward_r.benefit_type = 2 AND a_reward_r.category = cal_gs.category AND a_reward_r.partner = cal_gs.partner LEFT JOIN agent_rates AS p_rebate_r ON cal_gs.parent_id = p_rebate_r.agent_id AND p_rebate_r.benefit_type = 1 AND p_rebate_r.category = cal_gs.category AND p_rebate_r.partner = cal_gs.partner LEFT JOIN agent_rates AS p_reward_r ON cal_gs.parent_id = p_reward_r.agent_id AND p_reward_r.benefit_type = 2 AND p_reward_r.category = cal_gs.category AND p_reward_r.partner = cal_gs.partner ) AS total_gs ON a.id = total_gs.agent_id LEFT JOIN ( SELECT agent_costs.agent_id, ROUND(agent_costs.bonus_cost, 2) AS bonus_cost, agent_costs.agent_bonus_cost_rate, ROUND( ( agent_costs.bonus_cost * agent_costs.agent_bonus_cost_rate / 100 ), 2 ) AS take_bonus_cost, ROUND(agent_costs.event_cost, 2) AS event_cost, agent_costs.agent_event_cost_rate, ROUND( ( agent_costs.event_cost * agent_costs.agent_event_cost_rate / 100 ), 2 ) AS take_event_cost, ROUND( ( agent_costs.bonus_cost + agent_costs.event_cost ), 2 ) AS cost, ROUND( ( agent_costs.bonus_cost * agent_costs.agent_bonus_cost_rate / 100 ) + ( agent_costs.event_cost * agent_costs.agent_event_cost_rate / 100 ), 2 ) AS take_cost, ROUND( agent_costs.bonus_cost * ( agent_costs.parent_bonus_cost_rate - agent_costs.agent_bonus_cost_rate ) / 100, 2 ) AS parent_take_bonus_cost, ROUND( agent_costs.event_cost * ( agent_costs.parent_event_cost_rate - agent_costs.agent_event_cost_rate ) / 100, 2 ) AS parent_take_event_cost, ROUND( ( agent_costs.bonus_cost * ( agent_costs.parent_bonus_cost_rate - agent_costs.agent_bonus_cost_rate ) / 100 ) + ( agent_costs.event_cost * ( agent_costs.parent_event_cost_rate - agent_costs.agent_event_cost_rate ) / 100 ), 2 ) AS parent_take_cost, agent_costs.parent_bonus_cost_rate AS parent_bonus_cost_rate, agent_costs.parent_event_cost_rate AS parent_event_cost_rate FROM ( SELECT users.real_agent_id AS agent_id, SUM( CASE WHEN bonus_stats.reward_type LIKE 'Reward::CheckIn%' OR bonus_stats.reward_type LIKE 'Reward::LuckyWheel%' OR bonus_stats.reward_type LIKE 'Reward::Login%' OR bonus_stats.reward_type LIKE 'Reward::Roulette%' OR bonus_stats.reward_type LIKE 'Reward::CheckInMilestone%' OR bonus_stats.reward_type LIKE 'Reward::Rebate%' OR bonus_stats.reward_type LIKE 'Reward::RaffleEvent%' OR bonus_stats.reward_type LIKE 'Reward::LoginMilestone%' OR bonus_stats.reward_type LIKE 'Reward::UserLevel%' OR bonus_stats.reward_type LIKE 'Reward::Jackpot%' OR bonus_stats.reward_type LIKE 'Reward::ActiveUser%' THEN bonus_stats.cost_amount ELSE 0 END ) AS event_cost, SUM( CASE WHEN bonus_stats.reward_type NOT LIKE 'Reward::CheckIn%' AND bonus_stats.reward_type NOT LIKE 'Reward::LuckyWheel%' AND bonus_stats.reward_type NOT LIKE 'Reward::Login%' AND bonus_stats.reward_type NOT LIKE 'Reward::Roulette%' AND bonus_stats.reward_type NOT LIKE 'Reward::CheckInMilestone%' AND bonus_stats.reward_type NOT LIKE 'Reward::Rebate%' AND bonus_stats.reward_type NOT LIKE 'Reward::RaffleEvent%' AND bonus_stats.reward_type NOT LIKE 'Reward::LoginMilestone%' AND bonus_stats.reward_type NOT LIKE 'Reward::UserLevel%' AND bonus_stats.reward_type NOT LIKE 'Reward::Jackpot%' AND bonus_stats.reward_type NOT LIKE 'Reward::ActiveUser%' THEN bonus_stats.cost_amount ELSE 0 END ) AS bonus_cost, COALESCE(acr_bonus.percentage, 0) AS agent_bonus_cost_rate, COALESCE(acr_event.percentage, 0) AS agent_event_cost_rate, pcr_bonus.percentage AS parent_bonus_cost_rate, pcr_event.percentage AS parent_event_cost_rate FROM `bonus_stats` INNER JOIN `users` ON `users`.`id` = `bonus_stats`.`user_id` INNER JOIN `agents` ON `agents`.`id` = `users`.`real_agent_id` LEFT JOIN agent_cost_rates AS acr_bonus ON agents.id = acr_bonus.agent_id AND acr_bonus.cost_type = 'bonus' LEFT JOIN agent_cost_rates AS acr_event ON agents.id = acr_event.agent_id AND acr_event.cost_type = 'event' LEFT JOIN agent_cost_rates AS pcr_bonus ON agents.agent_id = pcr_bonus.agent_id AND pcr_bonus.cost_type = 'bonus' LEFT JOIN agent_cost_rates AS pcr_event ON agents.agent_id = pcr_event.agent_id AND pcr_event.cost_type = 'event' WHERE `bonus_stats`.`day` = '2024-01-01' GROUP BY users.real_agent_id ) AS agent_costs ) AS total_bs ON a.id = total_bs.agent_id WHERE a.level = 0 AND ( total_gs.agent_id IS NOT NULL OR total_bs.agent_id IS NOT NULL )",
	}, {
