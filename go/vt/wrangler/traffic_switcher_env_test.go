...
const (
	...
	streamExtInfoQuery = "select id, source, pos, stop_pos, max_replication_lag, state, db_name, time_updated, transaction_timestamp, time_heartbeat, time_throttled, component_throttled, message, tags, workflow_type, workflow_sub_type, rows_copied from _vt.vreplication where db_name = 'vt_%s' and workflow = '%s'"
