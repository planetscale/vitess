...
func (si *schemaInit) getCurrentSchema(tableName string) (string, error) {
	...
	rs, err := si.exec(si.ctx, sqlparser.BuildParsedQuery(showCreateTableQuery, sidecar.GetIdentifier(), sqlparser.String(sqlparser.NewIdentifierCS(tableName))).Query, 1, false)
	if err != nil {
