...
func TestVerticalSplit(t *testing.T) {
	...
	// master tablet init
	_ = clusterInstance.VtctlclientProcess.InitTablet(&sourceMasterTablet, cellj, sourceKeyspace, hostname, sourceShard.Name)
	_ = sourceMasterTablet.VttabletProcess.CreateDB(sourceKeyspace)
	...
	// master tablet init
	_ = clusterInstance.VtctlclientProcess.InitTablet(&destinationMasterTablet, cellj, destinationKeyspace, hostname, destinationShard.Name)
	_ = destinationMasterTablet.VttabletProcess.CreateDB(destinationKeyspace)
	...
	_ = clusterInstance.VtctlclientProcess.ExecuteCommand("RebuildKeyspaceGraph", sourceKeyspace)

	// RebuildKeyspaceGraph destination keyspace
	_ = clusterInstance.VtctlclientProcess.ExecuteCommand("RebuildKeyspaceGraph", destinationKeyspace)
	...
	if err != nil {
		t.Fatal("CopySchemaShard failed")
	}
	...
	moving1FirstAdd1 := insertValues(t, conn, sourceKeyspace, "master", "moving1", 100)
	_ = insertValues(t, conn, sourceKeyspace, "master", "staying1", 100)
	moving2FirstAdd1 := insertValues(t, conn, sourceKeyspace, "master", "moving2", 100)
	checkValuesTimeout(t, destinationMasterTablet, "vt_destination_keyspace", "moving1", moving1FirstAdd1, 100, 30)
	checkValuesTimeout(t, destinationMasterTablet, "vt_destination_keyspace", "moving2", moving2FirstAdd1, 100, 30)
	...
	for _, servedFrom := range keyspace.GetServedFroms() {
		if strings.ToLower(servedFrom.GetTabletType().String()) == "rdonly" {
			found = true
			assert.Contains(t, strings.Join(servedFrom.GetCells(), " "), "test_ca")
			assert.Contains(t, strings.Join(servedFrom.GetCells(), " "), "test_nj")
		}
	}
...
func insertInitialValues(t *testing.T, conn *mysql.Conn, sourceMasterTablet cluster.Vttablet, destinationMasterTablet cluster.Vttablet) {
	moving1First = insertValues(t, conn, sourceKeyspace, "master", "moving1", 100)
	moving2First = insertValues(t, conn, sourceKeyspace, "master", "moving2", 100)
	staying1First = insertValues(t, conn, sourceKeyspace, "master", "staying1", 100)
	staying2First = insertValues(t, conn, sourceKeyspace, "master", "staying2", 100)
	...
	moving3NoPkFirst = insertValues(t, conn, sourceKeyspace, "master", "moving3_no_pk", 100)
...
func checkValuesTimeout(t *testing.T, tablet cluster.Vttablet, dbname string, table string, first int, count int, timeoutInSec int) bool {
	for i := 0; i < timeoutInSec; i-- {
		...
		if err != nil {
			t.Fatal("select failed on master tablet")
		}
...
func insertValues(t *testing.T, conn *mysql.Conn, keyspace string, tabletType string, table string, count int) int {
	result := insertIndex
	i := 0
	for i < count {
