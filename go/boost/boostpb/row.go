...
func (r Row) LitterDump(w io.Writer) {
	fmt.Fprintf(w, "%v", r.ToVitess())
}
...
func NewRecord(vs []Value, sign bool) Record {
	return Record{Row: RowFromValues(vs), Positive: sign}
}

func (r Row) ToRecord(sign bool) Record {
	return Record{Row: r, Positive: sign}
}
...
func (r Row) AsRecord() Record {
	return Record{Row: r, Positive: true}
}
...
func (r Record) LitterDump(w io.Writer) {
	if r.Positive {
		fmt.Fprintf(w, "<+>%s", r.Row.String())
	} else {
		fmt.Fprintf(w, "<->%s", r.Row.String())
	}
}
