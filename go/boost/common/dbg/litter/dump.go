...
func (s *DumpState) indent() {
	if s.config.IndentPrefix != "" {
		s.writeString(s.config.IndentPrefix)
	}
	if s.config.Indent != "" {
		s.writeString(strings.Repeat(s.config.Indent, s.depth))
	}
}

func (s *DumpState) newlineWithPointerNameComment() {
	if ptr := s.currentPointer; s.config.PointerComments && ptr != nil {
		if s.config.Indent == "" {
...
func (s *DumpState) dumpSlice(v reflect.Value) {
	...
	for i := 0; i < numEntries; i++ {
		s.indent()
		s.dumpVal(v.Index(i))
		if s.config.Indent != "" || i < numEntries-1 {
			s.write([]byte(","))
		}
		s.newlineWithPointerNameComment()
	}
...
func (s *DumpState) dumpStruct(v reflect.Value) {
	...
	for i := 0; i < numFields; i++ {
		...
		s.dumpVal(v.Field(i))
		if s.config.Indent != "" || i < numFields-1 {
			s.write([]byte(","))
		}
		s.newlineWithPointerNameComment()
	}
...
func (s *DumpState) dumpMap(v reflect.Value) {
	...
	for i, key := range keys {
		...
		if s.config.Indent != "" || i < numKeys-1 {
			s.write([]byte(","))
		}
		s.newlineWithPointerNameComment()
	}
