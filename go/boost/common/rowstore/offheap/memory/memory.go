...
type Allocator struct {
	...
	cap    [128]int
	lists  [128]uintptr         // *node
	Mmaps  int                  // Asked from OS.
	pages  [128]uintptr         // *page
...
func (a *Allocator) newPage(size int) (uintptr /* *page */, error) {
	...
	(*page)(unsafe.Pointer(p)).class = 0
	return p, nil
}

func (a *Allocator) newSharedPage(class uint8) (uintptr /* *page */, error) {
	if a.cap[class] == 0 {
		a.cap[class] = int(pageAvail) / classSize(class)
	}
	size := int(headerSize) + a.cap[class]*classSize(class)
	p, err := a.mmap(size)
	if err != nil {
		return 0, err
	}
	...
	a.pages[class] = p
	(*page)(unsafe.Pointer(p)).class = class
	return p, nil
}
...
func (a *Allocator) UintptrFree(p uintptr) (err error) {
	...
	class := (*page)(unsafe.Pointer(pg)).class
	if class == 0 {
		if counters {
	...
	for i := 0; i < (*page)(unsafe.Pointer(pg)).brk; i++ {
		n := pg + headerSize + uintptr(i*classSize(class))
...
func (a *Allocator) UintptrMalloc(size int) (r uintptr, err error) {
	...
	if p := a.pages[class]; p != 0 {
		...
		offset := uintptr(((*page)(unsafe.Pointer(p)).brk - 1) * classSize(class))
		return p + headerSize + offset, nil
	}
...
func usableSize(p uintptr) (r int) {
	pg := p &^ uintptr(pageMask)
	if class := (*page)(unsafe.Pointer(pg)).class; class != 0 {
