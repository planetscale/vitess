...
func TestUMalloc(t *testing.T) {
	var alloc Allocator
	p, err := alloc.UintptrMalloc(maxSmallSize)
	if err != nil {
		t.Fatal(err)
	}

	pg := (*page)(unsafe.Pointer(p &^ uintptr(osPageMask)))
	if classSize(pg.class) > maxSmallSize {
		t.Fatal(classSize(pg.class), maxSmallSize)
	}
...
func TestMalloc(t *testing.T) {
	var alloc Allocator
	b, err := alloc.Malloc(maxSmallSize)
	if err != nil {
		t.Fatal(err)
	}

	p := (*page)(unsafe.Pointer(uintptr(unsafe.Pointer(&b[0])) &^ uintptr(osPageMask)))
	if classSize(p.class) > maxSmallSize {
		t.Fatal(classSize(p.class), maxSmallSize)
	}
